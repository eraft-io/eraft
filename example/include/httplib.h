...
using ContentReceiverWithProgress = std::function<bool(const char *data,
                                                       size_t      data_length,
                                                       uint64_t    offset,
...
class Server {
 ...
 private:
  ...
  bool handle_file_request(const Request &req,
                           Response      &res,
                           bool           head = false);
  bool dispatch_request(Request        &req,
                        Response       &res,
  ...
  void apply_ranges(const Request &req,
                    Response      &res,
                    std::string   &content_type,
  ...
  bool write_response(Stream   &strm,
                      ...
                      Request  &req,
                      Response &res);
  bool write_response_with_content(Stream        &strm,
                                   bool           close_connection,
                                   const Request &req,
                                   Response      &res);
...
class ClientImpl {
 ...
 private:
  ...
  bool                      read_response_line(Stream        &strm,
                                               const Request &req,
                                               Response      &res) const;
  bool                      write_request(Stream  &strm,
                                          Request &req,
                                          bool     close_connection,
                                          Error   &error);
  bool                      redirect(Request &req, Response &res, Error &error);
  bool                      handle_request(Stream   &strm,
                                           Request  &req,
                                           Response &res,
                                           bool      close_connection,
                                           Error    &error);
...
class decompressor {
 public:
  ...
  typedef std::function<bool(const char *data, size_t data_len)> Callback;
  virtual bool decompress(const char *data,
                          size_t      data_length,
                          Callback    callback) = 0;
};
...
class gzip_decompressor final : public decompressor {
 public:
  ...
  bool decompress(const char *data,
                  size_t      data_length,
                  Callback    callback) override;
...
inline bool from_hex_to_i(const std::string &s,
                          size_t             i,
                          size_t             cnt,
                          int               &val) {
  if (i >= s.size()) {
...
inline std::pair<size_t, size_t> trim(const char *b,
                                      const char *e,
                                      size_t      left,
                                      size_t      right) {
...
inline void split(const char                                     *b,
                  const char                                     *e,
                  char                                            d,
                  std::function<void(const char *, const char *)> fn) {
...
inline stream_line_reader::stream_line_reader(Stream &strm,
                                              char   *fixed_buffer,
                                              size_t  fixed_buffer_size)
    : strm_(strm)
...
inline bool mmap::open(const char *path) {
  ...
  hFile_ = ::CreateFile2(
      wpath.c_str(), GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING, NULL);
#else
  hFile_ = ::CreateFileW(wpath.c_str(),
                         GENERIC_READ,
                         FILE_SHARE_READ,
                         NULL,
                         OPEN_EXISTING,
  ...
  hMapping_ = ::CreateFileMappingW(
      hFile_, NULL, PAGE_READONLY, size.HighPart, size.LowPart, NULL);
#endif
...
inline ssize_t send_socket(socket_t    sock,
                           const void *ptr,
                           size_t      size,
                           int         flags) {
  return handle_EINTR([&]() {
...
inline Error wait_until_socket_is_ready(socket_t sock,
                                        time_t   sec,
                                        time_t   usec) {
#ifdef CPPHTTPLIB_USE_POLL
  struct pollfd pfd_read;
...
#ifdef SOCK_CLOEXEC
    auto sock = socket(
        hints.ai_family, hints.ai_socktype | SOCK_CLOEXEC, hints.ai_protocol);
#else
  ...
  for (auto rp = result; rp; rp = rp->ai_next) {
    ...
    if (rp->ai_family == AF_INET6) {
      ...
      setsockopt(sock,
                 IPPROTO_IPV6,
                 IPV6_V6ONLY,
                 reinterpret_cast<const char *>(&no),
                 sizeof(no));
#else
      setsockopt(sock,
                 IPPROTO_IPV6,
  ...
  auto sock = create_socket(
      ...
      [&](socket_t sock2, struct addrinfo &ai, bool &quit) -> bool {
        ...
        if (ret < 0) {
          ...
          error = wait_until_socket_is_ready(
...
inline constexpr unsigned int str2tag_core(const char  *s,
                                           size_t       l,
                                           unsigned int h) {
  return (l == 0)
             ? h
...
inline bool nocompressor::compress(const char *data,
                                   size_t      data_length,
                                   bool /*last*/,
                                   Callback callback) {
  if (!data_length) {
...
inline bool gzip_compressor::compress(const char *data,
                                      size_t      data_length,
                                      bool        last,
                                      Callback    callback) {
  assert(is_valid_);
...
inline bool gzip_decompressor::decompress(const char *data,
                                          size_t      data_length,
                                          Callback    callback) {
  assert(is_valid_);
...
inline bool brotli_compressor::compress(const char *data,
                                        size_t      data_length,
                                        bool        last,
                                        Callback    callback) {
...
inline bool brotli_decompressor::decompress(const char *data,
  ...
  while (decoder_r == BROTLI_DECODER_RESULT_NEEDS_MORE_OUTPUT) {
    ...
    decoder_r =
        BrotliDecoderDecompressStream(decoder_s,
                                      &avail_in,
                                      &next_in,
                                      &avail_out,
...
inline bool read_content_with_length(Stream                     &strm,
                                     uint64_t                    len,
                                     Progress                    progress,
...
template <typename T, typename U>
bool prepare_content_receiver(T                          &x,
                              int                        &status,
...
inline bool write_content(Stream                &strm,
  ...
  return write_content(
      strm, content_provider, offset, length, is_shutting_down, error);
}
...
inline bool write_content_chunked(Stream                &strm,
  ...
  data_sink.write = [&](const char *d, size_t l) -> bool {
    if (ok) {
      ...
      if (compressor.compress(
              d, l, false, [&](const char *data, size_t data_len) {
  ...
  auto done_with_trailer = [&](const Headers *trailer) {
    ...
    std::string payload;
    if (!compressor.compress(
            nullptr, 0, true, [&](const char *data, size_t data_len) {
              payload.append(data, data_len);
              return true;
            })) {
      ok = false;
...
inline bool write_content_chunked(Stream                &strm,
                                  ...
                                  U                     &compressor) {
  auto error = Error::Success;
  return write_content_chunked(
      strm, content_provider, is_shutting_down, compressor, error);
}
...
inline void parse_query_text(const char *data,
                             std::size_t size,
                             Params     &params) {
...
class MultipartFormDataParser {
 ...
 private:
  ...
  // Buffer
  bool start_with(const std::string &a,
                  size_t             spos,
                  size_t             epos,
                  const std::string &b) const {
    if (epos - spos < b.size()) {
...
inline std::string random_string(size_t length) {
  ...
  static std::seed_seq seed_sequence{
      seed_gen(), seed_gen(), seed_gen(), seed_gen()};
...
inline void hosted_at(const std::string        &hostname,
  ...
  for (auto rp = result; rp; rp = rp->ai_next) {
    ...
    std::string ip;
    auto        dummy = -1;
    if (detail::get_ip_and_port(
            addr, sizeof(struct sockaddr_storage), ip, dummy)) {
      addrs.push_back(ip);
    }
  }
...
inline ssize_t SocketStream::read(char *ptr, size_t size) {
  ...
  if (size < read_buff_size_) {
    auto n = read_socket(
        sock_, read_buff_.data(), read_buff_size_, CPPHTTPLIB_RECV_FLAGS);
    if (n <= 0) {
      return n;
...
inline bool PathParamsMatcher::match(Request &request) const {
  ...
  for (size_t i = 0; i < static_fragments_.size(); ++i) {
    ...
    // comparison
    if (std::strncmp(request.path.c_str() + starting_pos,
                     fragment.c_str(),
                     fragment.length()) != 0) {
      return false;
    }
...
inline bool Server::bind_to_port(const std::string &host,
                                 int                port,
                                 int                socket_flags) {
...
inline bool Server::listen(const std::string &host,
                           int                port,
                           int                socket_flags) {
  auto se = detail::scope_exit([&]() { done_ = true; });
...
inline bool Server::parse_request_line(const char *s, Request &req) const {
  ...
  static const std::set<std::string> methods{"GET",
                                             "HEAD",
                                             "POST",
                                             "PUT",
                                             "DELETE",
...
inline bool Server::write_response(Stream   &strm,
                                   bool      close_connection,
                                   Request  &req,
                                   Response &res) {
...
inline bool Server::write_content_with_provider(
  ...
  if (res.content_length_ > 0) {
    if (req.ranges.empty()) {
      return detail::write_content(strm,
      ...
      return detail::write_multipart_ranges_data(strm,
                                                 req,
                                                 res,
                                                 boundary,
                                                 content_type,
    ...
    if (res.is_chunked_content_provider_) {
      ...
      return detail::write_content_chunked(
          strm, res.content_provider_, is_shutting_down, *compressor);
    } else {
...
inline bool Server::read_content_core(
  ...
  if (req.is_multipart_form_data()) {
    ...
    out = [&](const char *buf, size_t n, uint64_t /*off*/, uint64_t /*len*/) {
      ...
      return true;
      */
      return multipart_form_data_parser.parse(
          buf, n, multipart_receiver, multipart_header);
    };
  } else {
    out = [receiver](
              const char *buf, size_t n, uint64_t /*off*/, uint64_t /*len*/) {
  ...
  if (!detail::read_content(
          strm, req, payload_max_length_, res.status, nullptr, out, true)) {
    return false;
  }
...
inline bool Server::handle_file_request(const Request &req,
                                        Response      &res,
                                        bool           head) {
...
inline int Server::bind_internal(const std::string &host,
                                 int                port,
                                 int                socket_flags) {
  if (!is_valid()) {
    return -1;
...
#endif
        auto val = detail::select_read(
            svr_sock_, idle_interval_sec_, idle_interval_usec_);
        if (val == 0) {  // Timeout
          task_queue->on_idle();
          continue;
        }
  ...
  if (res.body.empty()) {
    ...
    if (req.ranges.empty() || res.status != StatusCode::PartialContent_206) {
      ...
      detail::make_multipart_ranges_data(
          req, res, boundary, content_type, res.body.size(), data);
      res.body.swap(data);
    }
  ...
  auto ret = detail::process_server_socket(
      ...
      [this](Stream &strm, bool close_connection, bool &connection_closed) {
        return process_request(
            strm, close_connection, connection_closed, nullptr);
      });
...
inline bool ClientImpl::read_response_line(Stream        &strm,
                                           const Request &req,
                                           Response      &res) const {
  std::array<char, 2048> buf{};
...
inline bool ClientImpl::handle_request(Stream   &strm,
                                       Request  &req,
                                       Response &res,
                                       bool      close_connection,
                                       Error    &error) {
...
inline bool ClientImpl::write_request(Stream  &strm,
                                      Request &req,
                                      bool     close_connection,
                                      Error   &error) {
  // Prepare additional headers
...
inline Result ClientImpl::send_with_content_provider(
  ...
  auto res =
      send_with_content_provider(req,
                                 body,
                                 content_length,
  ...
  if ((res.status != StatusCode::NoContent_204) && req.method != "HEAD" &&
    ...
    auto out =
        req.content_receiver
            ...
            : static_cast<ContentReceiverWithProgress>([&](const char *buf,
                                                           size_t      n,
                                                           uint64_t /*off*/,
                                                           uint64_t /*len*/) {
...
inline Result ClientImpl::Get(const std::string &path,
                              ...
                              Progress           progress) {
  return Get(
      path, headers, nullptr, std::move(content_receiver), std::move(progress));
}
...
inline Result ClientImpl::Get(const std::string &path,
  ...
  req.content_receiver = [content_receiver](const char *data,
                                            size_t      data_length,
                                            uint64_t /*offset*/,
                                            uint64_t /*total_length*/) {
...
inline Result ClientImpl::Post(const std::string &path,
  ...
  auto query = detail::params_to_query_str(params);
  return Post(
      path, headers, query, "application/x-www-form-urlencoded", progress);
}
...
inline Result ClientImpl::Delete(const std::string &path,
                                 ...
                                 Progress           progress) {
  return Delete(
      path, Headers(), body.data(), body.size(), content_type, progress);
}
...
inline SSL *ssl_new(socket_t    sock,
                    SSL_CTX    *ctx,
                    std::mutex &ctx_mutex,
...
inline void ssl_delete(std::mutex &ctx_mutex,
                       SSL        *ssl,
                       socket_t    sock,
                       bool        shutdown_gracefully) {
...
inline SSLServer::SSLServer(X509       *cert,
                            EVP_PKEY   *private_key,
...
inline void SSLServer::update_certs(X509       *cert,
                                    EVP_PKEY   *private_key,
                                    X509_STORE *client_ca_cert_store) {
  ...
  if (ssl) {
    ret = detail::process_server_socket_ssl(
        ...
        [this, ssl](
          ...
          return process_request(
              strm, close_connection, connection_closed, [&](Request &req) {
                req.ssl = ssl;
              });
        });
...
inline SSLClient::SSLClient(const std::string &host,
  ...
  detail::split(
      &host_[0], &host_[host_.size()], '.', [&](const char *b, const char *e) {
  ...
  if (!client_cert_path.empty() && !client_key_path.empty()) {
    ...
    if (SSL_CTX_use_certificate_file(
            ctx_, client_cert_path.c_str(), SSL_FILETYPE_PEM) != 1 ||
        SSL_CTX_use_PrivateKey_file(
            ctx_, client_key_path.c_str(), SSL_FILETYPE_PEM) != 1) {
      SSL_CTX_free(ctx_);
      ctx_ = nullptr;
    }
  }
...
inline bool SSLClient::connect_with_proxy(Socket   &socket,
  ...
  if (!detail::process_client_socket(socket.sock,
                                     ...
                                     [&](Stream &strm) {
                                       ...
                                       return process_request(
                                           strm, req2, proxy_res, false, error);
                                     })) {
...
inline bool SSLClient::load_certs() {
  ...
  std::call_once(initialize_cert_, [&]() {
    ...
    if (!ca_cert_file_path_.empty()) {
      ...
      if (!SSL_CTX_load_verify_locations(
              ctx_, nullptr, ca_cert_dir_path_.c_str())) {
        ret = false;
      }
    } else {
      auto loaded = false;
...
inline void SSLClient::shutdown_ssl_impl(Socket &socket,
  ...
  if (socket.ssl) {
    detail::ssl_delete(
        ctx_mutex_, socket.ssl, socket.sock, shutdown_gracefully);
    socket.ssl = nullptr;
  }
  assert(socket.ssl == nullptr);
}
...
inline bool SSLClient::verify_host_with_common_name(X509 *server_cert) const {
  ...
  if (subject_name != nullptr) {
    char name[BUFSIZ];
    auto name_len = X509_NAME_get_text_by_NID(
        subject_name, NID_commonName, name, sizeof(name));
...
inline Client::Client(const std::string &host,
                      ...
                      const std::string &client_cert_path,
                      const std::string &client_key_path)
    : cli_(detail::make_unique<ClientImpl>(host,
                                           port,
                                           client_cert_path,
                                           client_key_path)) {}
...
inline Result Client::Get(const std::string &path,
                          ...
                          Progress           progress) {
  return cli_->Get(
      path, headers, std::move(content_receiver), std::move(progress));
}
inline Result Client::Get(const std::string &path,
                          ResponseHandler    response_handler,
                          ContentReceiver    content_receiver) {
  return cli_->Get(
      path, std::move(response_handler), std::move(content_receiver));
}
inline Result Client::Get(const std::string &path,
                          const Headers     &headers,
                          ResponseHandler    response_handler,
                          ContentReceiver    content_receiver) {
  return cli_->Get(
      path, headers, std::move(response_handler), std::move(content_receiver));
}
inline Result Client::Get(const std::string &path,
                          ResponseHandler    response_handler,
                          ContentReceiver    content_receiver,
...
inline Result Client::Post(const std::string &path,
                           ...
                           Progress           progress) {
  return cli_->Post(
      path, headers, body, content_length, content_type, progress);
}
inline Result Client::Post(const std::string &path,
                           const std::string &body,
                           const std::string &content_type) {
...
inline Result Client::Post(const std::string &path,
                           ...
                           const std::string &content_type) {
  return cli_->Post(
      path, content_length, std::move(content_provider), content_type);
}
inline Result Client::Post(const std::string           &path,
                           ContentProviderWithoutLength content_provider,
                           const std::string           &content_type) {
...
inline Result Client::Post(const std::string &path,
                           ...
                           const std::string &content_type) {
  return cli_->Post(
      path, headers, content_length, std::move(content_provider), content_type);
}
inline Result Client::Post(const std::string           &path,
                           const Headers               &headers,
                           ContentProviderWithoutLength content_provider,
