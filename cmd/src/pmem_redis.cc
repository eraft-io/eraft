...
bool PMemRedis::_Init()
{
	...
	// init raftstore
	std::shared_ptr<network::RaftConfig> raftConf = std::make_shared<network::RaftConfig>(g_config.listenAddr, g_config.dbPath, g_config.nodeId);

	std::unique_ptr<network::RaftStack> raftStack(new network::RaftStack(raftConf));
	raftStack->Start();
