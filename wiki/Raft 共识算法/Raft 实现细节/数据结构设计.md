...
PEERS[peers: RaftPeer数组<br/>对等节点列表]
...
LOGS[logs: Entry数组<br/>日志数组]
COMMIT_INDEX[commitIndex: int<br/>提交索引]
LAST_APPLIED[lastApplied: int<br/>最后应用索引]
NEXT_INDEX[nextIndex: int数组<br/>下一个索引]
MATCH_INDEX[matchIndex: int数组<br/>匹配索引]
ELECTION_TIMER[electionTimer: Timer<br/>选举计时器]
HEARTBEAT_TIMER[heartbeatTimer: Timer<br/>心跳计时器]
end
subgraph "并发访问控制"
...
REPLICATOR_COND[replicatorCond: Cond数组] --> COND
```
...
InitHeartbeatTimer --> READ_PERSIST[读取持久化状态]
