...
func MakeClerk(servers []string) *Clerk {
	...
	for i, addr := range servers {
		if strings.Contains(addr, ":") || strings.HasPrefix(addr, "localhost") {
...
func MakeClerkWithMakeEnd(servers []string, makeEnd func(string) *labrpc.ClientEnd) *Clerk {
...
func (ck *Clerk) Command(request *CommandRequest) Config {
	...
	for {
		...
		if ck.clients[ck.leaderId] == nil && ck.makeEnd != nil {
			// Initialize labrpc client if needed
			ck.clients[ck.leaderId] = &LabrpcShardCtrlerClient{end: ck.makeEnd(ck.servers[ck.leaderId])}
		}
