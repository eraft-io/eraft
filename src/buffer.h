...
class CircularBuffer {
 public:
  ...
  bool IsFull() const {
    return ((writePos_ + 1) & (maxSize_ - 1)) == readPos_;
  }

  void GetDatum(BufferSequence& buffer,
                std::size_t     maxSize,
                std::size_t     offset = 0);
  ...
  std::size_t WriteableSize() const {
    return maxSize_ - ReadableSize() - 1;
  }
...
template <typename BUFFER>
bool CircularBuffer<BUFFER>::PushDataAt(const void* pData,
                                        std::size_t nSize,
                                        std::size_t offset) {
  if (!pData || 0 == nSize)
    return true;
  ...
  if (readPos > writePos) {
    ...
    if (availBytes1 >= nSize + 1) {
      ...
      if (bytesLeft > 0) {
        ::memcpy(&buffer_[0],
                 static_cast<const char*>(pData) + availBytes1,
                 bytesLeft);
      }
    }
  }
...
template <typename BUFFER>
bool CircularBuffer<BUFFER>::PeekDataAt(void*       pBuf,
                                        std::size_t nSize,
                                        std::size_t offset) {
  if (!pBuf || 0 == nSize) {
    return true;
  ...
  if (readPos < writePos) {
    ...
    if (availBytes1 >= nSize) {
      ...
      ::memcpy(static_cast<char*>(pBuf) + availBytes1,
               &buffer_[0],
               nSize - availBytes1);
    }
  }
