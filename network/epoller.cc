...
bool AddSocket(int epfd, int socket, uint32_t events, void *ptr) {
  ...
  ev.events = 0;
  if (events & EventTypeRead) ev.events |= EPOLLIN;
  if (events & EventTypeWrite) ev.events |= EPOLLOUT;
...
bool Epoller::AddSocket(int sock, int events, void *userPtr) {
  if (Epoll::AddSocket(multiplexer_, sock, events, userPtr)) return true;
...
bool Epoller::ModSocket(int sock, int events, void *userPtr) {
  if (events == 0) return DelSocket(sock, 0);

  if (Epoll::ModSocket(multiplexer_, sock, events, userPtr)) return true;

  return errno == ENOENT && AddSocket(sock, events, userPtr);
}

int Epoller::Poll(std::vector<FiredEvent> &events, size_t maxEvent,
                  int timeoutMs) {
  ...
  if (nFired == -1 && errno != EINTR && errno != EWOULDBLOCK) return -1;
  ...
  for (int i = 0; i < nFired; ++i) {
    ...
    if (events_[i].events & EPOLLIN) fired.events |= EventTypeRead;
    if (events_[i].events & EPOLLOUT) fired.events |= EventTypeWrite;
