...
QGlobRegex::QGlobRegex(const char* pattern, std::size_t plen, const char* text,
                       std::size_t tlen) {
...
bool QGlobRegex::TryMatch() {
  while (pOff_ < pLen_) {
    switch (pattern_[pOff_]) {
      ...
      case '?':
        if (!_ProcessQuestion()) return false;

        break;

      case '[':
        if (!_ProcessBracket()) return false;

        break;

      case '\\':
        if (pOff_ + 1 < pLen_ && pattern_[pOff_ + 1] == '[') ++pOff_;
        // fall through;

      default:
        if (tOff_ >= tLen_) return false;

        if (pattern_[pOff_] != text_[tOff_]) return false;
...
bool QGlobRegex::_IsMatch() const { return pOff_ == pLen_ && tLen_ == tOff_; }
