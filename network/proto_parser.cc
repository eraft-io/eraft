...
ParseResult GetIntUntilCRLF(const char *&ptr, std::size_t nBytes, int &val) {
  if (nBytes < 3) return ParseResult::wait;
  ...
  for (; i < nBytes; ++i) {
    if (isdigit(ptr[i])) {
      ...
      if (i + 1 == nBytes) return ParseResult::wait;
...
ParseResult ProtoParser::ParseRequest(const char *&ptr, const char *end) {
  if (multi_ == -1) {
    ...
    if (parseRet != ParseResult::ok) return ParseResult::wait;
  }

  return _ParseStrlist(ptr, end, params_);
}

ParseResult ProtoParser::_ParseMulti(const char *&ptr, const char *end,
                                     int &result) {
  if (end - ptr < 3) return ParseResult::wait;

  if (*ptr != '*') return ParseResult::error;
...
ParseResult ProtoParser::_ParseStrlist(const char *&ptr, const char *end,
  ...
  while (static_cast<int>(numOfParam_) < multi_) {
    if (results.size() < numOfParam_ + 1) results.resize(numOfParam_ + 1);
...
ParseResult ProtoParser::_ParseStr(const char *&ptr, const char *end,
                                   std::string &result) {
  if (paramLen_ == -1) {
...
ParseResult ProtoParser::_ParseStrval(const char *&ptr, const char *end,
  ...
  if (static_cast<int>(end - ptr) < paramLen_ + 2) return ParseResult::wait;
...
ParseResult ProtoParser::_ParseStrlen(const char *&ptr, const char *end,
                                      int &result) {
  if (end - ptr < 3) return ParseResult::wait;
