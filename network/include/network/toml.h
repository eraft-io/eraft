...
// Formatting flags
enum FormatFlag { FORMAT_NONE = 0, FORMAT_INDENT = 1 };
...
public:
	...
	Value(std::string &&v)
	    : type_(STRING_TYPE), string_(new std::string(std::move(v)))
	{
	}
	...
	void write(std::ostream *, const std::string &keyPrefix = std::string(),
		   int indent = -1) const;
...
private:
	...
	template <typename T>
	friend struct ValueConverter;
};

// parse() returns ParseResult.
struct ParseResult {
	ParseResult(toml::Value v, std::string er)
	    : value(std::move(v)), errorReason(std::move(er))
	{
	}
...
public:
	...
	Token(TokenType tokenType, std::chrono::system_clock::time_point tp)
	    : type_(tokenType), time_value_(tp)
	{
	}
	...
	std::chrono::system_clock::time_point timeValue() const
	{
		return time_value_;
	}
...
public:
	explicit Parser(std::istream &is) : lexer_(is), token_(TokenType::ERROR_TOKEN)
	{
		if (!lexer_.skipUTF8BOM()) {
			token_ = Token(TokenType::ERROR_TOKEN,
				       std::string("Invalid UTF8 BOM"));
		} else {
			nextKey();
		}
	}
...
private:
	...
	void nextKey()
	{
		token_ = lexer_.nextKeyToken();
	}
	void nextValue()
	{
		token_ = lexer_.nextValueToken();
	}
...
inline ParseResult parse(std::istream &is)
{
	if (!is) {
		return ParseResult(toml::Value(),
				   "stream is in bad state. file does not exist?");
	}
...
inline Token Lexer::nextStringDoubleQuote()
{
	if (!consume('"'))
		return Token(TokenType::ERROR_TOKEN,
			     std::string("string didn't start with '\"'"));
		...
		if (c == '\\') {
			if (!current(&c))
				return Token(
					...
					std::string(
						"string has unknown escape sequence"));
			next();
			switch (c) {
				case 't':
					c = '\t';
			...
			if (multiline) {
				if (current(&c) && c == '"') {
					...
					if (current(&c) && c == '"') {
						next();
						return Token(TokenType::MULTILINE_STRING,
							     s);
					} else {
						s += '"';
						s += '"';
...
inline Token Lexer::nextStringSingleQuote()
{
	...
	if (current(&c) && c == '\'') {
		...
		return Token(TokenType::ERROR_TOKEN,
			     std::string("string didn't end with '\'\'\'' ?"));
	}
...
inline Token Lexer::parseAsTime(const std::string &str)
{
	...
	// TODO(mayah): workaround GCC 4.9.3 on cygwin does not have std::round, but
	// round().
...
inline Value::Value(const Value &v) : type_(v.type_)
{
	switch (v.type_) {
		case NULL_TYPE:
			null_ = v.null_;
...
inline Value::Value(Value &&v) noexcept : type_(v.type_)
{
	switch (v.type_) {
		case NULL_TYPE:
			null_ = v.null_;
...
struct Value::ValueConverter<double> {
	bool is(const Value &v)
	{
		return v.type() == Value::DOUBLE_TYPE;
	}
...
struct Value::ValueConverter<std::string> {
	bool is(const Value &v)
	{
		return v.type() == Value::STRING_TYPE;
	}
...
struct Value::ValueConverter<Array> {
	bool is(const Value &v)
	{
		return v.type() == Value::ARRAY_TYPE;
	}
...
struct Value::ValueConverter<Table> {
	bool is(const Value &v)
	{
		return v.type() == Value::TABLE_TYPE;
	}
...
template <>
inline const char *type_name<int64_t>()
{
	return "int64_t";
}
template <>
inline const char *type_name<double>()
{
	return "double";
}
template <>
...
template <>
inline const char *type_name<toml::Time>()
{
	return "time";
}
template <>
inline const char *type_name<toml::Array>()
{
	return "array";
}
template <>
inline const char *type_name<toml::Table>()
{
	return "table";
}
} // namespace internal
...
{
	if (!is<T>())
		failwith("type error: this value is ", typeToString(type_), " but ",
			 internal::type_name<T>(), " was requested");
}
...
inline double Value::asNumber() const
{
	...
	failwith("type error: this value is ", typeToString(type_),
		 " but number is requested");
}
		...
		case TABLE_TYPE:
			for (const auto &kv : *table_) {
				if (kv.second.is<Table>())
					continue;
				if (kv.second.is<Array>() && kv.second.size() > 0 &&
				    kv.second.find(0)->is<Table>())
					continue;
...
inline const Value *Value::find(const std::string &key) const
{
	...
	while (true) {
		...
		if (!(t.type() == internal::TokenType::IDENT ||
		      t.type() == internal::TokenType::STRING))
			return nullptr;
	...
	while (true) {
		...
		if (token().type() == TokenType::DOT) {
			...
			if (Value *candidate = currentValue->findChild(key)) {
				if (candidate->is<Array>() && candidate->size() > 0)
					candidate =
						candidate->find(candidate->size() - 1);
				if (!candidate->is<Table>())
					return nullptr;
		...
		if (token().type() == TokenType::RBRACKET) {
			...
			if (Value *candidate = currentValue->findChild(key)) {
				...
				if (isArray) {
					currentValue =
						currentValue->setChild(key, Array());
					currentValue = currentValue->push(Table());
				} else {
					currentValue =
						currentValue->setChild(key, Table());
				}
			}
			break;
		}
